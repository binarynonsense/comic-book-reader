...
parentPort.on("message", async (msg) => {
  if (msg.type === "shutdown") {
    ...
    try {
      let resizeNeeded = msg.resizeNeeded;
      let imageOpsNeeded = msg.imageOpsNeeded;
      let uiSelectedOptions = msg.uiOptions;
      ...
      if (
        ...
        if (imageFormat != FileExtension.NOT_SET) {
          ...
          if (imageFormat === FileExtension.JPG) {
            ...
            if (
              parseInt(uiSelectedOptions.outputImageFormatParams.pngQuality) <
              100
            ) {
              pipeline.png({
                quality: parseInt(
