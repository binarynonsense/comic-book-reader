...
async function fillPublishers() {
  ...
  try {
    const response = await net.get(
      "https://digitalcomicmuseum.com/preview/index.php#publishers",
      { timeout: 15000 },
    );
    const parser = new DOMParser().parseFromString(response.data, "text/html");
    const publisherElements = parser.querySelectorAll(
      ".browser-publisher-card",
    );
    ...
    for (let index = 0; index < publisherElements.length; index++) {
      let aElement = publisherElements[index];
      let parts = aElement.href.split("cid=");
      g_publishersSelect.innerHTML += `<option value="${parts[1]}">${aElement.children[1].children[0].innerHTML}</option>`;
    }
    document
      .querySelector("#tool-dcm-catalog-error-message-div")
      .classList.add("set-display-none");
...
async function fillComics(titleId) {
  ...
  try {
    ...
    for (let index = 0; index < comicElements.length; index++) {
      let aElement = comicElements[index];
      let cardTitle = aElement.querySelector(".browser-card-title");
      g_comicsSelect.innerHTML += `<option value="${cardTitle.href.split("did=")[1]}">${cardTitle.innerText}</option>`;
    }
    document
      .querySelector("#tool-dcm-catalog-error-message-div")
      .classList.add("set-display-none");
