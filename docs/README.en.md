...
> Note: For Linux distributions that require setting an AppArmor profile for Electron apps to run, like Ubuntu 24.04, it's best to choose a version other than the regular or AppImage ones as they probably won't work on them by default due to that restriction.
